...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE routine_name = 'event_check_version_on_create'
      AND routine_schema = 'cuisine_production'
  ) THEN
    RAISE NOTICE 'Routine event_check_version_on_create EXISTS';
  ELSE
    CREATE FUNCTION cuisine_production.event_check_version_on_create() RETURNS trigger
  ...
  ELSE
    CREATE FUNCTION cuisine_production.event_immutable() RETURNS trigger
      LANGUAGE plpgsql AS $_$
    BEGIN
  ...
  ELSE
    CREATE FUNCTION cuisine_production.event_not_deletable() RETURNS trigger
      LANGUAGE plpgsql AS $_$
    BEGIN
      RAISE EXCEPTION 'not allowed';
    END;
