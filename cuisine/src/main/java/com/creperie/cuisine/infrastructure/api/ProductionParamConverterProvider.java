...
@Provider
public class ProductionParamConverterProvider implements ParamConverterProvider {

    private static final Map<Class<?>, Function<String, ?>> CONVERTERS = Map.of(
            PreparationIdentifier.class, PreparationIdentifier::from
    );

    private static final Map<Class<?>, Function<Object, String>> SERIALIZERS = Map.of(
            PreparationIdentifier.class, value -> ((PreparationIdentifier) value).id()
    );
